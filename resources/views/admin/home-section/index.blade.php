        ...
        <div class="card-body">
            <ul class="nav nav-tabs" id="myTab" role="tablist">
                <li class="nav-item" role="presentation">
                  <a class="nav-link active" id="home-tab" data-toggle="tab" href="#home" role="tab" aria-controls="home" aria-selected="true">{{ $websiteLang->where('lang_key','about_us')->first()->custom_text }}
                  </a>
                </li>
                ...
                <li class="nav-item" role="presentation">
                  <a class="nav-link" id="urgent-tab" data-toggle="tab" href="#urgent" role="tab" aria-controls="profile" aria-selected="false">{{ $websiteLang->where('lang_key','urgent_properties')->first()->custom_text }}</a>
                </li>
              ...
              <div class="tab-content" id="myTabContent">
                ...
                <div class="tab-pane fade mt-5" id="urgent" role="tabpanel" aria-labelledby="urgent-tab">
                    @php
                    $urgentProperties=$sections->where('id',12)->first();
                    @endphp
                    <div class="row">
                        <div class="col-md-8">
                            <form action="{{ route('admin.feature-in-homepage',$urgentProperties->id) }}" method="post">
                                @csrf
                                <div class="form-group">
                                    <label for="header">{{ $websiteLang->where('lang_key','header')->first()->custom_text }}</label>
                                    <input type="text" class="form-control" name="header" id="header" value="{{ $urgentProperties->header }}">
                                </div>

                                <div class="form-group">
                                    <label for="description">{{ $websiteLang->where('lang_key','des')->first()->custom_text }}</label>
                                    <textarea class="form-control" cols="30" rows="5"  id="description" name="description">{{ $urgentProperties->description }}</textarea>
                                </div>

                                <div class="form-group">
                                    <label for="content_quantity">{{ $websiteLang->where('lang_key','content_qty')->first()->custom_text }}</label>
                                    <input type="number" name="content_quantity" id="content_quantity" class="form-control" value="{{ $urgentProperties->content_quantity }}">
                                </div>

                                <div class="form-group">
                                    <label for="show_homepage">{{ $websiteLang->where('lang_key','show_homepage')->first()->custom_text }}</label>
                                    <select name="show_homepage" id="show_homepage" class="form-control">
                                        <option {{ $urgentProperties->show_homepage==1 ? 'selected':'' }} value="1">{{ $websiteLang->where('lang_key','yes')->first()->custom_text }}</option>
                                        <option {{ $urgentProperties->show_homepage==0 ? 'selected':'' }} value="0">{{ $websiteLang->where('lang_key','no')->first()->custom_text }}</option>
                                    </select>
                                </div>
                                <button type="submit" class="btn btn-success">{{ $websiteLang->where('lang_key','update')->first()->custom_text }}</button>
                ...
                <div class="tab-pane fade mt-5" id="listings" role="tabpanel" aria-labelledby="listings-tab">
                   ...
                   <div class="row">
                       <div class="col-md-8">
                           <form action="{{ route('admin.banner-award-in-homepage',$serviceSection->id) }}" method="post">
                               @csrf
