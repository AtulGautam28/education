                ...
                <div class="card-body">
                    <form action="{{ route('admin.about.update',$about->id) }}" enctype="multipart/form-data" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="about" id="summernote" class="summernote">{{ $about->about }}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="service">{{  $websiteLang->where('lang_key','service')->first()->custom_text }}</label>
                            <textarea name="service" id="summernote3" class="summernote1">{{ $about->service }}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="history">{{  $websiteLang->where('lang_key','history')->first()->custom_text }}</label>
                            <textarea name="history" id="summernote2" class="summernote2">{{ $about->history }}</textarea>
                        </div>
